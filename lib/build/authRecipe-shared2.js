 ...
 * Imports.
 */
class AuthRecipe extends index.RecipeModule {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    getAuthRecipeDefaultRedirectionURL = async (_context) => {
        throw new Error("Should never come here");
    };
    signOut = async (input) => {
        return await types.Session.getInstanceOrThrow().signOut({
            userContext: genericComponentOverrideContext.getNormalisedUserContext(input?.userContext),
        });
    };
    doesSessionExist = async (input) => {
        return await types.Session.getInstanceOrThrow().doesSessionExist({
            userContext: genericComponentOverrideContext.getNormalisedUserContext(input?.userContext),
        });
    };
}
