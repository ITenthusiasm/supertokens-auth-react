...
const [useContext, Provider$1] = genericComponentOverrideContext.createGenericComponentsOverrideContext();
...
class Provider {
    ...
    getRedirectURL() {
        ...
        const callbackPath = new NormalisedURLPath__default.default(`/callback/${this.config.id}`);
    ...
    getButton = (name) => {
        if (this.config.buttonComponent !== undefined) {
            if (typeof this.config.buttonComponent === "function") {
                return jsxRuntime.jsx(this.config.buttonComponent, { name: name ?? this.name });
            }
 ...
 * Class.
 */
class ActiveDirectory extends Provider {
    /*
     * Static Attributes.
     */
    static instance;
            ...
            children: [
                ...
                jsxRuntime.jsx("defs", {
                    children: jsxRuntime.jsx("clipPath", {
                        id: "clip0_402_84",
                        children: jsxRuntime.jsx("rect", { width: "416", height: "415", fill: "white" }),
                    }),
                }),
            ],
        });
...
class Apple extends Provider {
    ...
    getRedirectURIOnProviderDashboard() {
        ...
        const callbackPath = new NormalisedURLPath__default$1.default(`/callback/${this.id}`);
        const path = genericComponentOverrideContext.SuperTokens.getInstanceOrThrow()
 ...
 * Class.
 */
class BoxySAML extends Provider {
    /*
     * Static Attributes.
     */
    static instance;
        ...
        return jsxRuntime.jsxs("svg", {
            ...
            children: [
                ...
                jsxRuntime.jsx("defs", {
                    children: jsxRuntime.jsx("clipPath", {
                        id: "clip0_4108_67056",
                        children: jsxRuntime.jsx("rect", { width: "18", height: "14", fill: "white" }),
                    }),
                }),
            ],
        });
...
class GoogleWorkspaces extends Provider {
    static instance;
    constructor(config) {
        super({
...
const getFunctionOverrides = (recipeId, onHandleEvent) => (originalImp) => ({
    ...originalImp,
...
function matchRecipeIdUsingState(recipe, userContext) {
    const stateResponse = recipe.webJSRecipe.getStateAndOtherInfoFromStorage({
        userContext,
    });
...
async function redirectToThirdPartyLogin(input) {
    const loginMethods =
        await genericComponentOverrideContext.Multitenancy.getInstanceOrThrow().getCurrentDynamicLoginMethods({
            userContext: input.userContext,
        });
    const tenantProviders = loginMethods?.firstFactors.includes(types.FactorIds.THIRDPARTY)
...
const mergeProviders = ({ tenantProviders = [], clientProviders = [] }) => {
    ...
    const usesDynamicLoginMethods = genericComponentOverrideContext.SuperTokens.usesDynamicLoginMethods === true;
    ...
    for (const tenantProvider of tenantProviders) {
        ...
        if (provider !== undefined) {
            ...
            if (builtInProvidersMap[providerID]) {
                const provider = new builtInProvidersMap[providerID]({
                    id: tenantProvider.id,
                    name: tenantProvider.name,
                });
 ...
 * Class.
 */
class ThirdParty extends index.AuthRecipe {
    webJSRecipe;
    static instance;
    static RECIPE_ID = "thirdparty";
    recipeID = ThirdParty.RECIPE_ID;
    firstFactorIds = [types.FactorIds.THIRDPARTY];
    getFirstFactorsForAuthPage() {
        return this.firstFactorIds;
    }
    constructor(config, webJSRecipe = ThirdpartyWebJS__default.default) {
        if (
